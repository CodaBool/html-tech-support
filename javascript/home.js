...
function validateState() {
	var errorDiv = document.getElementById("stateErrorMessage");
	var input = document.getElementById("state");
    try {
        if (input.value == "") {
            // Custom checked but message box is empty.
            throw "This field is required."
        } else if (!input.value.includes("FL") && !input.value.includes("Florida") && !input.value.includes("fl")) {
			throw "Unfortunately, you are outside our jurisdiction."
		} else { //clear error display
            errorDiv.style.display = "none";
            input.style.background = "white";
        }
